...
import { uploadFileToS3, isS3Configured } from "@/utils/s3";
...
export async function POST(
  ...
  let pdfUrl = `/invoices/${invoiceNumber}.pdf`;
  if (isS3Configured()) {
    const uploaded = await uploadFileToS3(pdfPath, `${invoiceNumber}.pdf`);
    if (uploaded) {
      pdfUrl = uploaded;
    }
  }

  invoiceDoc.pdfUrl = pdfUrl;
